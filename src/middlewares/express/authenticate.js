...
async function isAuthenticated(req, res, next) {
  try {
    ...
    if (token === null) return res.status(401).json({ err: error.jwt.notFound });
    
    const isValid = isValidToken(authHeader);
    if (!isValid || req.session.data === undefined) return res.status(401).json({ err: error.notAuthenticated });
    next();
  }catch(err) {
